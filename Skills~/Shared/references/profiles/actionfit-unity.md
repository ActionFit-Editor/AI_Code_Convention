...
`AFCC-RFS-001` is deprecated. Existing mixed public `refs` fields may remain until an explicitly approved serialization migration, but new or deliberately revised inputs use `AFCC-RFS-002`, `AFCC-RFS-005`, and `AFCC-SER-004`. `AFCC-RFS-003` preserves the legacy `RequiredReference` plus `AutoWireChild` pair, and `AFCC-RFS-004` preserves the consumer-enqueued integrated selector contract; both are deprecated compatibility rules.
...
`com.actionfit.referencebinding` is a required dependency of this package rather than an optional capability gate. Under `AFCC-RFS-005`, every newly created or deliberately revised mandatory `Refs` Component field in a supported ReferenceBinding shape uses `[SerializeField, RequiredChildReference("ExactGameObjectName")]`. `SerializeField` owns private-field serialization and Inspector exposure. `RequiredChildReference` supplies default-Inspector read-only presentation, required validation with no caller-defined error code, and authoritative exact-name child wiring. The selector is the exact, case-sensitive root-or-descendant GameObject name. The package-owned Editor pump discovers and routes loaded owners, so consumers do not add `OnValidate` solely for ReferenceBinding. A custom consuming asmdef must explicitly reference `com.actionfit.referencebinding`; package installation does not rewrite asmdefs. Existing `AFCC-RFS-003` fields using `RequiredReference` and `AutoWireChild` together and existing `AFCC-RFS-004` consumer-enqueued integrated declarations remain supported but are not the new-authoring standard. `Assets` may use `RequiredReference` for mandatory references but never `AutoWireChild` or `RequiredChildReference`; ReferenceBinding does not search the AssetDatabase or prove that a reference is persistent. Optional stable child Component selectors may still use `AutoWireChild` without required validation. Editor-only attributes and tools may author private fields before Play Mode, but the installed owner must reject Player and Play-Mode mutation paths.

Resolve selector ambiguity before authoring. Use an existing valid serialized reference or an explicit user decision to identify the intended candidate. When the same Component type and name still collide, do not select the first result or omit `RequiredChildReference`. Rename only non-selected duplicates, only inside an explicitly authorized asset-edit scope, with deterministic collision-free temporary names derived from the original name and stable hierarchy order. Report the old/new hierarchy paths, search for runtime name lookups, Animation bindings, and external tool contracts, inspect the targeted serialized diff, and verify reload without reference loss. Without intended-candidate evidence, asset-name edit authority, or validation evidence, stop and report the ambiguity. Untouched legacy fields remain outside automatic migration.
...
- For `AFCC-RFS-005`, verify every newly created or deliberately revised supported mandatory `Refs` Component field has `[SerializeField, RequiredChildReference("ExactGameObjectName")]` and does not add `OnValidate` solely for ReferenceBinding; verify duplicate-name remediation has explicit asset authority, reported path mappings, targeted diff inspection, and reload evidence. Existing `AFCC-RFS-003` pairs and `AFCC-RFS-004` consumer-enqueued declarations remain compatibility evidence, not new-authoring failures.
