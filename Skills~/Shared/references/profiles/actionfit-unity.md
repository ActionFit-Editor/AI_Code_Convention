...
`AFCC-RFS-001` is deprecated. Existing mixed public `refs` fields may remain until an explicitly approved serialization migration, but new or deliberately revised inputs use `AFCC-RFS-002`, `AFCC-RFS-004`, and `AFCC-SER-004`. `AFCC-RFS-003` is also deprecated and preserves the legacy `RequiredReference` plus `AutoWireChild` pair for existing declarations.
...
- For `AFCC-RFS-004`, verify every newly created or deliberately revised supported mandatory `Refs` Component field has `[SerializeField, RequiredChildReference("ExactGameObjectName")]` and an Editor-guarded owner enqueue; verify duplicate-name remediation has explicit asset authority, reported path mappings, targeted diff inspection, and reload evidence. Existing `AFCC-RFS-003` pairs remain compatibility evidence, not new-authoring failures.
