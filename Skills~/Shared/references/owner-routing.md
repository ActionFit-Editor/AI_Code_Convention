...
For the serialized-reference route, `AFCC-RFS-004` selects the installed owner's `RequiredChildReference` contract for newly created or deliberately revised supported mandatory child Component fields. Existing `AFCC-RFS-003` `RequiredReference` plus `AutoWireChild` pairs remain compatibility declarations, while external mandatory assets keep the owner-documented `RequiredReference` contract. This routing text does not redefine those APIs or authorize migration.
