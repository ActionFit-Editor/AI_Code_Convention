...
7. Compare documented rules and owned API contracts, not every source file. For `AFCC-TRE-001`, `AFCC-PKG-001`, and `AFCC-PRT-001`, compare only documented architecture/package intent and explicitly cited owner contracts. Do not infer source violations, inventory assemblies, or produce a project refactoring roadmap; route that separate request to `$refactor-plan` when installed. Use exactly these relationship categories:
