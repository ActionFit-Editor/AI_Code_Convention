...
Convention template은 예시 `Refs`, `Assets`, `Settings` container, private 직렬화 backing field와 getter 전용 접근을 가진 새 `MonoBehaviour`를 생성합니다. 필수 child Component인 `Refs.contentRoot` 예시는 `[SerializeField, RequiredChildReference("ContentRoot")]`를 사용합니다. `SerializeField`가 private field의 직렬화와 Inspector 노출을 담당하고, 통합 어트리뷰트는 기본 Inspector에서 읽기 전용으로 표시하면서 별도 수동 에러 코드 없이 필수 검증과 정확한 이름의 authoritative child wiring을 제공합니다. 외부 asset인 `Assets.iconSprite`는 hierarchy wiring 없이 `RequiredReference("ICON_SPRITE_MISSING")`를 유지합니다. Package-owned ReferenceBinding Editor pump가 owner를 자동 감지하므로 template은 `OnValidate`를 생성하지 않습니다. 선택한 대상, 이름 변경 상호작용, `#SCRIPTNAME#` 치환과 root namespace 확장은 Unity가 소유합니다.

`com.actionfit.referencebinding@0.2.2`는 필수 의존성입니다. owner Runtime 어셈블리가 auto-reference되므로 생성된 스크립트는 Unity predefined assembly에서 컴파일됩니다. 사용하는 custom asmdef는 `com.actionfit.referencebinding`을 명시적으로 참조해야 하며 generator는 프로젝트 asmdef를 변경하지 않습니다. 메뉴 실행은 `AFCC-PRO-001` profile selector를 선택하거나 기존 스크립트를 변경하거나 Scene/Prefab을 저장하거나 이후 편집의 준수를 증명하지 않습니다.
...
- `AFCC-RFS-003`은 기존 `RequiredReference`+`AutoWireChild` 쌍의 의미와 호환성을 보존하는 deprecated 규칙입니다.
- `AFCC-RFS-004`는 integrated attribute와 consumer `OnValidate` enqueue를 함께 요구하던 기존 의미를 보존하는 deprecated 규칙입니다.
- `AFCC-RFS-005`는 새로 만들거나 의도적으로 수정하는 지원 가능한 필수 `Refs` Component 필드에 `[SerializeField, RequiredChildReference("ExactGameObjectName")]`를 요구하고 package-owned Editor pump에 자동 처리를 맡깁니다. Consumer는 ReferenceBinding만을 위해 `OnValidate`를 추가하지 않습니다. 기존 필드는 일괄 마이그레이션하지 않으며, 이름 중복은 승인된 에셋 범위에서만 결정론적 임시 이름으로 해소하고 변경 경로를 사용자에게 보고합니다.
- `AFCC-DOC-001`은 비단순 변경의 완료 전에 처음 라우팅한 문서, 관련 owner guide와 최종 diff가 새로 연관시킨 문서를 다시 읽고, 저장소가 선언한 문서 validator를 실행하도록 요구합니다. 의미 재확인과 기계 검증은 서로를 대체하지 않습니다.
- `AFCC-INT-001`에 따라 `actionfit-unity` profile은 구체적 소유권을 우선합니다. 증거가 있는 외부 계약, 교체 가능한 운영 구현, 플랫폼/runtime 변형 또는 구현 없는 불가피한 어셈블리 경계가 있을 때만 새 interface를 허용합니다. 기존 interface가 자동 마이그레이션 대상이 되지는 않습니다.
...
Custom Package Manager `1.1.106` 이상은 변경되거나 충돌하는 target을 보존하면서 schema v2 skill을 프로젝트 로컬 agent 폴더에 설치합니다.
...
- `$code-convention-apply`: 선택한 profile과 설치된 API owner를 해석한 뒤 사용자가 구체적인 Unity 코드 변경을 승인한 경우에만 유효 convention을 적용하고, 완료 전 문서 재확인과 저장소 validator 결과를 요구합니다.
