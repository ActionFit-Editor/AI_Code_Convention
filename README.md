...
```json
{
  "dependencies": {
    ...
    "com.actionfit.referencebinding": "https://github.com/ActionFit-Editor/ReferenceBinding.git#0.2.0",
    "com.actionfit.ai-codeconvention": "https://github.com/ActionFit-Editor/AI_Code_Convention.git#0.5.0"
  }
}
```
...
- `AFCC-RFS-003`은 새로 만들거나 의도적으로 수정하는 지원 가능한 필수 `Refs` Component 필드에 `RequiredReference`, authoritative `AutoWireChild`, Editor-guarded owner enqueue를 함께 요구합니다. 기존 필드는 일괄 마이그레이션하지 않으며, 이름 중복은 승인된 에셋 범위에서만 결정론적 임시 이름으로 해소하고 변경 경로를 사용자에게 보고합니다.
